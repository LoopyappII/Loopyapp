...
import { ArrowLeft, MapPin, LogIn, LogOut as LogOutIcon } from "lucide-react";
import { supabase } from "@/lib/supabaseClient";
import { haversineMeters } from "@/lib/geo";
import type { Loop, LoopMember, SafeZone } from "@/lib/types";
...
export default function LoopPage() {
  ...
  async function loadLoopData(uid: string) {
    ...
    const { data: memberRows } = await supabase
      .from("loop_members")
      .select("*, profiles!loop_members_user_id_fkey(id, name, avatar_url)")
      .eq("loop_id", loopId);
  ...
  // Geolocalización propia + push a Supabase + chequeo de zonas
  useEffect(() => {
            ...
            <ul className="text-sm space-y-1">
              {members.map((m) => (
                <li key={m.id} className="flex justify-between text-loopy-700">
                  <span>{m.profiles?.name || "Miembro"}</span>
                  <span className="text-xs text-bridge font-medium">{m.role}</span>
                </li>
              ))}
            </ul>
          </motion.div>
